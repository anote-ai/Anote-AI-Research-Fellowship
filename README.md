# Anote AI Research Fellowship — Summer 2026

**June 1 – August 10, 2026 | New York City + Virtual**

---

Anote is an applied AI research company based in New York City. We build AI technology to provide high-quality datasets and evaluations for leading enterprises, federal clients, and model providers.

The Anote AI Research Fellowship is a 10-week summer program bringing together a select cohort of student researchers and engineers to do real, publishable AI research — while directly contributing to Anote's products. Fellows work alongside the Anote team, guided by research mentors and a steering committee, with the goal of shipping meaningful work that lasts beyond the summer.

Fellows join us in person at our WeWork space in NYC every Friday, and participate virtually throughout the week. Remote fellows are welcome.

---

## What Fellows Work On

Every fellow is responsible for three core deliverables by August 10:

| # | Deliverable | Goal |
|---|-------------|------|
| 1 | **AI Research Paper** | Publish a paper to arXiv and submit to a top AI conference (e.g. AAAI 2027). Topics drawn from [anote-ai/Research](https://github.com/anote-ai/Research) or proposed by the fellow. |
| 2 | **Final Presentation** | Present the summer's work on August 5 — recorded and published to Anote's YouTube and website. Slides use the [Anote Template](https://docs.google.com/presentation/d/1x4E2dwBnDImHjMOMaI8lLWcRNjVWlnsYEJ3WOwlhKuA/edit?usp=sharing), inspired by past showcases like [Pale Blue Dot](https://anote.ai/palebluedot) and [Panacea](https://anote.ai/panacea). |
| 3 | **Product Contribution** | Own one Anote product and meaningfully improve it using Claude Code or Codex. All product repos live at [github.com/anote-ai](https://github.com/orgs/anote-ai/repositories). |

---

## Program Highlights

- **Orientation:** Monday, June 1 — 6:00–8:00 PM EST
- **Weekly Research Standup:** Every Tuesday, 5:30 PM EST - 6 PM EST (virtual)
- **Weekly Product Standup:** Every Thursday, 5:30 PM EST - 6 PM EST (virtual)
- **Fellowship AI Talks:** Every Friday, 6:00 PM EST - 7:00 PM EST — guest AI experts, recorded virtually
- **Midpoint Check-ins:** Wednesday, July 1 — one-on-one sessions with Natan (5:00–10:00 PM)
- **Final Presentations:** Wednesday, August 5 — recorded and published (5:00–10:00 PM)
- **Anote Summer Party:** Friday, August 7 — celebrating the fellows in NYC (6:00–9:00 PM)
- **Last Day:** Monday, August 10 — clean up all work, document everything, submit papers

---

## Repository Contents

| File | Description |
|------|-------------|
| [SUMMER_CALENDAR.md](./SUMMER_CALENDAR.md) | Full week-by-week schedule for the entire fellowship, including all fixed events, recurring sessions, one-on-one slot grids, and PM notes |
| [PRODUCT_FELLOW_MATCHING.md](./PRODUCT_FELLOW_MATCHING.md) | All 10 Anote products with repo links, descriptions, and fellow assignment column |
| [RESEARCH_FELLOW_MATCHING.md](./RESEARCH_FELLOW_MATCHING.md) | All 25 research paper tracks (active + backlog) with fellow assignment column, deliverable checklist, and conference deadline guide |
| [PRODUCT_MANAGER.md](./PRODUCT_MANAGER.md) | Full scope of work for the Fellowship PM — standups, speaker pipeline, onboarding, fellow support, marketing, playbook, and budget |
| [FELLOW_ROSTER.md](./FELLOW_ROSTER.md) | Fellow cohort roster — names, schools, research tracks, product assignments, GitHub handles, and access checklist |
| [ONBOARDING_CHECKLIST.md](./ONBOARDING_CHECKLIST.md) | Step-by-step PM checklist for onboarding every fellow before June 1, including orientation agenda |
| [STEERING_COMMITTEE.md](./STEERING_COMMITTEE.md) | Steering committee member list, responsibilities, engagement calendar, and paper-to-mentor assignments |
| [RECRUITMENT.md](./RECRUITMENT.md) | Application criteria, scoring rubric, interview process, sourcing channels, and cohort size guidance |
| [WELCOME_PACKET.pdf](./WELCOME_PACKET.pdf) | Welcome packet with all the information about Anote's AI Research Fellowship. |

---

## Anote Alumni & Past Success

The fellowship builds on a strong tradition of student researchers and engineers who have come through Anote. Many have gone on to roles at top AI companies, research labs, and PhD programs.

Explore past Anote alumni, coffee chats, and where they've landed: **[community.anote.ai/community/coffeechats](https://community.anote.ai/community/coffeechats)**

Past summer showcases that set the bar for what fellows can produce:
- [Pale Blue Dot](https://anote.ai/palebluedot)
- [Panacea](https://anote.ai/panacea)

---

## Key Links

| Resource | Link |
|----------|------|
| Anote Website | [anote.ai](https://anote.ai) |
| Anote GitHub | [github.com/anote-ai](https://github.com/orgs/anote-ai/repositories) |
| Research Repo | [anote-ai/Research](https://github.com/anote-ai/Research) |
| Anote Slides Template | [Google Slides](https://docs.google.com/presentation/d/1x4E2dwBnDImHjMOMaI8lLWcRNjVWlnsYEJ3WOwlhKuA/edit?usp=sharing) |
| GTM Proposals | [Google Sheets](https://docs.google.com/spreadsheets/d/1Yb3zE-xvDUHsfiRaR75ggmMmOmkkgiDhgJ3WaVtRp10/edit?usp=sharing) |

---

*Summer 2026 | Anote AI Research Fellowship*
